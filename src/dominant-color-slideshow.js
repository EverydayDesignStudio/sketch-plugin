...
export default function({ api, command, document, plugin, scriptPath, scriptURL, selection }) {
  ...
  sketch.UI.message("Still needs to be implemented")
  stop
  ...
  if (selection.count() != 1) {
    ...
    sketch.UI.message("Dominant Color for Slideshow")
    // sketch.UI.message("Jordan White this is running from the test.")
    stop
  } else {
    var inputString = sketch.UI.getStringFromUser(
      "Please paste in dominant color output\nExample data", "",
    )
